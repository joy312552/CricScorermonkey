...
export const BroadcastOverlayContainer: React.FC<BroadcastOverlayContainerProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-[100] overflow-hidden font-display">
      ...
      <AnimatePresence mode="wait">
        {activeCmd === 'TEAM_VS' && <TeamVS key="team_vs" match={match} />}
        ...
        {activeCmd === 'TOSS_WINNER' && <TossOverlay key="toss" match={match} visible={true} />}
      </AnimatePresence>

      {/* Layer 3: Lower Thirds & Side Panels */}
      <AnimatePresence>
        {activeCmd === 'BATT_SUMMARY' && <BatsmanCard key="batt" match={match} />}
        {activeCmd === 'BALL_SUMMARY' && <BowlingCard key="bowling" match={match} />}
        {activeCmd === 'BAR_STAT' && <ComparisonBarChart key="bar" match={match} />}
        {activeCmd === 'LINE_STAT' && <ComparisonLineChart key="line" match={match} />}
